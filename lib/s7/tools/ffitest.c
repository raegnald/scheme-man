...
static s7_pointer make_and_free(s7_scheme *sc)
{
  s7_scheme *s7;
  s7 = s7_init();
  s7_eval_c_string(s7, "(+ 1 1)"); /* or load some file? */
  s7_free(s7);
  return(s7_f(sc));
}
...
int main(int argc, char **argv)
{
  ...
  if (!s7_is_immutable(p))
    fprintf(stderr, "%d: s7_set_immutable failed?\n", __LINE__);
  s7_gc_unprotect_at(sc, gc_loc);
  p = s7_make_symbol(sc, "_asdf_");
  s7_set_immutable(sc, p); /* hits s7_slot with an undefined slot */
  if (s7_is_immutable(p))  /* p is the symbol -- see s7_set_immutable */
    fprintf(stderr, "%d: s7_set_immutable worked?\n", __LINE__);
  s7_symbol_set_value(sc, p, s7_nil(sc));  /* same as above */
  {
    s7_pointer p1 = s7_name_to_value(sc, "_asdf_");
    if (p1 != s7_undefined(sc))
      {fprintf(stderr, "%d: _asdf_: %s\n", __LINE__, s1 = TO_STR(p1)); free(s1);}
  }
  if (s7_is_defined(sc, "_asdf_")) /* same as above */
    {fprintf(stderr, "%d: _asdf_ is defined?: %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    ...
    fprintf(stderr, "s7_c_pointer_type is not ()\n");
  {
    ...
    csc = s7_make_c_pointer_wrapper_with_type(sc, (void *)sc, s7_make_symbol(sc, "s7_scheme*"), s7_f(sc));
    if (!s7_is_c_pointer_of_type(csc, s7_make_symbol(sc, "s7_scheme*")))
      {fprintf(stderr, "c-pointer wrapper type %s != s7_scheme*\n", s1 = TO_STR(s7_c_pointer_type(csc))); free(s1);}
  }
  ...
  s7_define_function_star(sc, "plus1", plus1, "a b c", "an example of define* from C");
  {
    ...
    p = s7_make_c_object_without_gc(sc, dax_type_tag, perm2 = (void *)malloc(sizeof(dax)));
    {
      ...
      gc_loc1 = s7_gc_protect(sc, port);
      s7_display(sc, s7_make_string(sc, "(define c_object_var 23)"), port);
      s7_newline(sc, port);
  ...
  s1 = TO_STR(s7_lambda_body(sc, p));
  if (strcmp(s1, "((+ a b (apply * c)))") != 0)
    {fprintf(stderr, "%d: s7_lambda_body is %s?\n", __LINE__, s1);}
  free(s1);

  s1 = TO_STR(s7_lambda_parameters(sc, p));
  if (strcmp(s1, "(a b . c)") != 0)
    {fprintf(stderr, "%d: s7_lambda_parameters is %s?\n", __LINE__, s1);}
  free(s1);

  s1 = TO_STR(s7_lambda_let(sc, p));
  if (strcmp(s1, "(rootlet)") != 0)
    {fprintf(stderr, "%d: s7_lambda_let is %s?\n", __LINE__, s1);}
  free(s1);

  if (s7_lambda_body(sc, s7_name_to_value(sc, "abs")) != s7_nil(sc))
    fprintf(stderr, "s7_lambda_body(abs) is not nil?\n");
  if (s7_lambda_parameters(sc, s7_name_to_value(sc, "abs"))  != s7_nil(sc))
    fprintf(stderr, "s7_lambda_parameters(abs) is not nil?\n");
  if (s7_lambda_let(sc, s7_name_to_value(sc, "abs"))  != s7_rootlet(sc))
    fprintf(stderr, "s7_lambda_let(abs) is not rootlet?\n");
